// Arithmetic Operators (+, -, *, /, %, **)
// These operators perform mathematical calculations on operands.

// Store the price of two books and calculate the total price.
let theHistoryBook = 299
let theScienceBook = 499
totalPriceOfBooks = theHistoryBook+theScienceBook //here the addition operations perform . 
console.log("The Total price of books is : ", totalPriceOfBooks)

// Divide ₹500 equally among 4 friends.
let totalAmount = 500
let friends = 4 
console.log("The 500 divide in 4 friend is : " ,totalAmount/friends, "per friend. ") //here the divide operator perform . 

// Find the remainder when 125 is divided by 12.
let value = 125
let dividedValue = 12 
console.log("The remainder is : ",value%dividedValue)

// Calculate the square of a number using **.
let squareNumber = 5
console.log("The square of number is : ",squareNumber**2)

// Calculate the cube of a number.
let cubeOfNumber = 5
console.log("The Cube of number is : ",cubeOfNumber**3)

// Convert 2 hours into minutes using multiplication.
let hours = 2
let minutes = 60
console.log("The 2 hours into minutes is : ",hours*minutes)

// Convert 5 days into hours.
let days = 5
let oneDayhour = 24
console.log("The 5 days into hours is : ", days * oneDayhour)

// Find the total seconds in 3 hours.
let hours2 = 3
let seconds = 3600 // 1 hours = 3600 sec
console.log("The 3 hours into secounds is : ", hours2 * seconds)

// Calculate the total marks of 5 subjects.
let subject1 = 45
let subject2 = 54
let subject3 = 22
let subject4 = 69
let subject5 = 99
let TotalMarks = subject1 + subject2 + subject3 + subject4 + subject5
console.log("The Total marks of 5 subjects is : ",TotalMarks)
// Find the average marks of 5 subjects.
let averageMarks = TotalMarks / 5
console.log("The Average marks of 5 subjects is : ",averageMarks)